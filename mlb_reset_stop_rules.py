#!/usr/bin/env python3
"""
Manual reset for MLB stop rules.

Appends a RESET event to suspension_log and clears all suspensions.
Does NOT delete history — log is append-only.

Usage:
    python mlb_reset_stop_rules.py --reason "line quality improved"
    python mlb_reset_stop_rules.py --status        # print current state, no change
"""
...
def status() -> None:
    s = _load()
    print(f"[mlb_stop_rules] model_suspended: {s.get('model_suspended', False)}")
    print(f"[mlb_stop_rules] suspended_tiers: {s.get('suspended_tiers', [])}")
    print(f"[mlb_stop_rules] suspension_log entries: {len(s.get('suspension_log', []))}")
...
def reset(reason: str = "") -> None:
    ...
    if not currently_suspended:
        print("[mlb_reset_stop_rules] Nothing is currently suspended — nothing to reset.")
        return
    ...
    print(f"[mlb_reset_stop_rules] All suspensions cleared at {now}")
