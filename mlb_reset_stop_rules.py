...
def reset(reason: str = "") -> None:
    ...
    if not state.get("suspended"):
        print("[mlb_reset_stop_rules] Model is not currently suspended — nothing to reset.")
        print(f"  State: {state}")
        return
    ...
    print(f"[mlb_reset_stop_rules] Suspension cleared at {now}")
    print(f"  Reason: {reason or 'manual reset'}")
    print(f"  Suspension log now has {len(state['suspension_log'])} entries.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Reset MLB stop-rule suspension")
