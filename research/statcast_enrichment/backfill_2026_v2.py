#!/usr/bin/env python3
"""
2026 Statcast Backfill v2 — per-start pitcher aggregation.
Fixes: Int64/nullable zone dtype -> uses .fillna(0) before .astype(int).
PIT-SAFE: re-checks 2022-2025 rows unchanged before writing.
"""
...
# ── fixed _aggregate_pitcher_start (handles nullable Int64 columns) ───────────
def _aggregate_pitcher_start(chunk_df):
    ...
    # Cast nullable integer columns to float/object to avoid NA-to-int errors
    for col in ["pitcher", "game_pk", "zone", "launch_speed", "launch_angle",
    ...
    df["is_hard_hit"] = (
        (df["launch_speed"] >= 95) & (df["is_bip"] == 1)
    ).fillna(False).astype(int) if "launch_speed" in df.columns else 0

    if "launch_speed" in df.columns and "launch_angle" in df.columns:
        df["is_barrel"] = (
            (df["launch_speed"] >= 98) &
            (df["launch_angle"] >= (30 - (df["launch_speed"].clip(upper=116) - 98).clip(lower=0)).clip(lower=26)) &
            (df["launch_angle"] <= (30 + (df["launch_speed"].clip(upper=116) - 98).clip(lower=0)).clip(upper=50)) &
            (df["is_bip"] == 1)
    ...
    if "zone" in df.columns:
        zone_f = df["zone"].astype(float)
        df["is_in_zone"]     = zone_f.between(1, 9).fillna(False).astype(int)
        df["is_outside_zone"] = (~zone_f.between(1, 9)).fillna(False).astype(int)
    else:
    ...
    df["is_chase"]      = ((df["is_outside_zone"] == 1) & (df["is_swing"] == 1)).fillna(False).astype(int)
    df["is_zone_swing"] = ((df["is_in_zone"]      == 1) & (df["is_swing"] == 1)).fillna(False).astype(int)
    df["is_zone_contact"] = ((df["is_zone_swing"] == 1) & (df["is_contact"] == 1)).fillna(False).astype(int)

    if "release_spin_rate" in df.columns and "pitch_type" in df.columns:
        df["spin_ff"] = np.where(df["pitch_type"].isin(["FF", "SI"]), df["release_spin_rate"].astype(float), np.nan)
        df["spin_sl"] = np.where(df["pitch_type"].isin(["SL", "ST", "SV"]), df["release_spin_rate"].astype(float), np.nan)
    else:
        df["spin_ff"] = np.nan
        df["spin_sl"] = np.nan
    ...
    # pitcher and game_pk are now float — use them as groupby keys
    ...
    # cast back to int64
    agg["pitcher_id"] = agg["pitcher_id"].astype("Int64").astype("int64")
    agg["game_pk"]    = agg["game_pk"].astype("Int64").astype("int64")
...
for chunk in chunks_2026:
    ...
    if chunk_path.exists():
        existing_chunk = pd.read_parquet(chunk_path)
        print(f"  {cid}: chunk already exists ({len(existing_chunk)} rows), using it")
        new_rows_total += len(existing_chunk)
        continue
    ...
    for attempt in range(3):
        try:
            ...
            if len(agg) > 0:
                ...
                print(f"    {cid}: {len(agg)} pitcher-starts saved")
...
# ── re-merge all chunks (skip _raw files) ────────────────────────────────────
...
for p in sorted(CHUNKS.glob("pitcher_statcast_*.parquet")):
    ...
    try:
        ...
        # normalise game_pk + pitcher_id to int64
