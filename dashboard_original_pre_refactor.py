...
def _render_nhl_tab() -> None:
    ...
    st.html('<div style="font-size:0.68em;color:#78716c;margin-bottom:4px;'
    ...
    def _nhl_universal_card(s: dict) -> None:
        ...
        if side and line is not None:
            unit_label = "1.0u" if is_play else "shadow"
            card_wagers.append(f"{side.upper()} {line} \u00b7 {unit_label}")
        elif side:
            unit_label = "1.0u" if is_play else "shadow"
            card_wagers.append(f"{side.upper()} \u00b7 {unit_label}")
        ...
        # Caution banner for 6.5-line overs (extra_html)
        caut = int(s.get("caution_flag") or 0)
        caution_html = ""
        if caut:
            caution_html = (
...
def _render_soccer_tab() -> None:
    ...
    # Classify signals
    active_sigs = [s for s in today_signals if s.get("confidence_tier") in ("HIGH", "MEDIUM")]
    shadow_sigs = [s for s in today_signals if s.get("confidence_tier") == "LOW"]
    other_sigs  = [s for s in today_signals if s not in active_sigs and s not in shadow_sigs]
    ...
    # ── helper: build universal soccer card ───────────────────────────────────
    def _soccer_universal_card(s: dict, card_tier: str) -> None:
        home   = s.get("home_team", "")
        away   = s.get("away_team", "")
        ...
        # Wager
        wagers = ["OVER 2.5"] if card_tier == "ACTIVE" else []

        # Pills
        pills = []
        lg_label = _soccer_league_label(league)
        if lg_label:
            pills.append(_universal_pill(lg_label, "#94a3b8", "#1e293b"))
        tier_color = {"HIGH": "#22c55e", "MEDIUM": "#eab308", "LOW": "#64748b"}.get(tier, "#64748b")
        tier_bg    = {"HIGH": "#052e16", "MEDIUM": "#1c1400", "LOW": "#0f172a"}.get(tier, "#0f172a")
        pills.append(_universal_pill(tier, tier_color, tier_bg))
        lineup_label = "\u2713 Lineup" if lineup else "? Lineup est."
        lineup_color = "#22c55e" if lineup else "#64748b"
        lineup_bg    = "#052e16" if lineup else "#1e293b"
        pills.append(_universal_pill(lineup_label, lineup_color, lineup_bg))

        # Stats
        edge_pp = f"{edge * 100:+.1f}pp" if edge is not None else "\u2014"
        stats = [f"Edge: {edge_pp}"]
        if model_t is not None: stats.append(f"Model: {model_t:.1f}")
        if over_p is not None: stats.append(f"Price: {over_p:.2f}")
        if gt: stats.append(f"KO: {gt}")

        # Move badge as extra_html
        move_html = ""
        if move is not None:
            move_s = f"{move:+.3f}" if move is not None else "\u2014"
            mc = "#22c55e" if (move or 0) >= 0 else "#f87171"
            move_html = (f'<div style="font-size:0.75em;color:{mc};margin-top:3px">'
                         f'Market move: {move_s}</div>')

        _render_game_card_universal(
            matchup=matchup, time_str="", tier=card_tier,
            wagers=wagers, pills=pills, stats=stats, weather="",
            extra_html=move_html,
        )

    # ── SECTION 1: Today's Plays ───────────────────────────────────────────────
    st.html(f'<div class="section-hdr">\U0001f3af Today\'s Plays \u2014 OVER 2.5</div>')
    if active_sigs:
        for s in active_sigs:
            _soccer_universal_card(s, "ACTIVE")
    else:
        st.caption(
            "No HIGH/MEDIUM signals today "
    ...
    if shadow_sigs:
        st.html('<hr style="border:none;border-top:1px solid #333;margin:12px 0 8px 0">'
                '<div style="font-size:0.72em;color:#94a3b8;font-weight:600;margin-bottom:6px">'
                '\u25d0 Shadow Monitoring</div>')
        for s in shadow_sigs:
            _soccer_universal_card(s, "SHADOW")
...
def _render_nba_tab() -> None:
    ...
    with col_title:
        if nba:
            ...
            st.html(f"<h3 style='margin:0 0 4px 0'>\U0001f3c0 NBA Totals</h3>"
                    f"<span style='font-size:0.78em;color:#4a5568'>"
                    f"Projections for <strong>{game_date}</strong></span>")
        else:
    ...
    if nba is None:
        st.info("No NBA projections available yet.")
        return
    ...
    if sa:
        _sa_parts = []
        for _k, _l in [("HIGH", "HIGH"), ("MEDIUM", "MEDIUM"), ("LOW", "LOW")]:
            _d = sa.get(_k, {})
            if _d and _d.get("n", 0) > 0:
                _w = _d.get("w", 0)
                _lo = _d.get("l", 0)
                _roi = _d.get("roi")
                _rs = f" ({_roi:+.1f}%)" if _roi is not None else ""
                _sa_parts.append(f"{_l}: {_w}-{_lo}{_rs}")
        if _sa_parts:
            st.html(f'<div style="font-size:0.78em;color:#94a3b8;margin-bottom:10px">'
                    f'Season: {" | ".join(_sa_parts)}</div>')

    # ── stop rule suspension banner ──
    _nba_stop = (nba or {}).get("stop_rule_status", {})
    if _nba_stop.get("model_suspended"):
        st.html('<div style="background:#2d1515;border:2px solid #dc2626;border-radius:8px;'
                'padding:12px 16px;margin-bottom:14px">'
                '<span style="color:#f87171;font-weight:700;font-size:1.05em">'
                '\U0001f6a8 NBA MODEL FULLY SUSPENDED</span></div>')
    for _t in _nba_stop.get("suspended_tiers", []):
        st.html(f'<div style="background:#2d1b0e;border:2px solid #d97706;border-radius:8px;'
                f'padding:12px 16px;margin-bottom:8px">'
                f'<span style="color:#fbbf24;font-weight:700">\u26d4 NBA {_t} tier suspended</span></div>')

    # ── TODAY'S PLAYS ──
    st.html(f'<div class="section-hdr">\U0001f3af Today\'s Plays</div>')
    if active:
        for g in active:
            ...
            lean = g.get("lean", "")
            line = g.get("line")
            sizing = g.get("final_sizing") or g.get("unit_size") or 0.75
            wagers = []
            if lean and line:
                wagers.append(f"{lean} {line} \u00b7 {sizing}u")
            elif lean:
                wagers.append(f"{lean} \u00b7 {sizing}u")
            ...
            po = g.get("p_over")
            if po is not None: stats.append(f"P(over): {po:.0%}")
            extra = ""
            inj = []
            ...
            _render_game_card_universal(
                matchup=matchup, time_str=time_str, tier="ACTIVE",
                wagers=wagers, pills=pills, stats=stats, weather="", extra_html=extra)
    else:
    ...
    # ── ALL OTHER GAMES ──
    if other:
        with st.expander(f"All other games \u2014 {len(other)}", expanded=False):
            for g in other:
                matchup = f"{g.get('away_team','')} @ {g.get('home_team','')}"
                time_str = g.get("game_time_et", "")
                stats = []
                ...
                if pred: stats.append(f"Proj: {pred:.1f}")
                _render_game_card_universal(
                    matchup=matchup, time_str=time_str, tier="NONE",
                    wagers=[], pills=[], stats=stats, weather="")

    # ── RECENT RESULTS ──
    recent = nba.get("recent_results", [])
    if recent:
        st.html('<div class="section-hdr">\U0001f4cb Recent Results (14 days)</div>')
        rows_html = ""
        for r in recent[:20]:
            res = r.get("result", "")
            rc = "#4ade80" if res == "WIN" else "#f87171" if res == "LOSS" else "#94a3b8"
            net = r.get("net_units")
            net_s = f"{net:+.2f}u" if net is not None else "\u2014"
            rows_html += (f'<tr style="color:{rc}">'
                         f'<td>{r.get("game_date","")}</td>'
                         f'<td>{r.get("away_team","")}@{r.get("home_team","")}</td>'
                         f'<td>{r.get("signal_type","")}</td>'
                         f'<td>{r.get("closing_total","")}</td>'
                         f'<td>{r.get("actual_total","")}</td>'
                         f'<td>{res}</td><td>{net_s}</td></tr>')
        st.html(f'<table class="star-table"><thead><tr>'
                f'<th>Date</th><th>Matchup</th><th>Signal</th>'
                f'<th>Line</th><th>Actual</th><th>Result</th><th>Net</th>'
                f'</tr></thead><tbody>{rows_html}</tbody></table>')

    # ── OT DIAGNOSTICS ──
    ot = nba.get("ot_diagnostics", {})
    if ot and ot.get("total_graded", 0) > 0:
        with st.expander("OT Diagnostics", expanded=False):
            st.html(f'<div style="font-size:0.78em;color:#94a3b8">'
                    f'OT games: {ot.get("ot_games",0)} / {ot.get("total_graded",0)} '
                    f'({ot.get("ot_rate","?")}%) | Flips: {ot.get("ot_flips",0)} '
                    f'({ot.get("ot_flip_rate","?")}%)</div>')

    # ── CLV SUMMARY ──
    clv = nba.get("clv_summary", {})
    if clv and clv.get("total_with_clv", 0) >= 20:
        with st.expander("CLV Summary", expanded=False):
            st.html(f'<div style="font-size:0.78em;color:#94a3b8">'
                    f'Avg CLV: {clv.get("avg_clv",0):+.3f} | '
                    f'Positive CLV: {clv.get("pct_positive_clv",0):.1f}% | '
                    f'N: {clv.get("total_with_clv",0)}</div>')

    # Review tab content moved to Tracker tab
...
def _render_mlb_tab(data: dict | None, stats: dict | None) -> None:
    ...
    try:
        ...
        # GREEN — active engines (only Signal B / F5 Run Line remains active)
        _green_pills = []
    ...
    try:
        ...
        if _tn > 0:
            st.html(
                ...
                f'\U0001f4ca Production Record (Mar 30+): '
...
def _render_tracker_tab() -> None:
    ...
    mlb_signals.append({"name": "V1 Legacy", "w": v1_w, "l": v1_l, "p": v1_p, "roi": v1_roi})
